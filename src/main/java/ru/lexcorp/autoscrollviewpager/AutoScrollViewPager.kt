 ...
 *  * [.interval] set auto scroll time in milliseconds, default is [.DEFAULT_INTERVAL]
 *
 *
 * **Advanced Settings and Usage**
 *  * [.way] set auto scroll way
 *  * [.cycle] set whether automatic cycle when auto scroll reaching the last or first item, default
 * is true
 *  * [.slideBorderMode] set how to process when sliding at the last or first item
 *  * [.stopScrollWhenTouch] set whether stop auto scroll when touching, default is true
 *
 */
class AutoScrollViewPager : ViewPager {
    ...
    /** auto scroll way, default is [.RIGHT]  */
    var way = RIGHT
     ...
     * [.SLIDE_BORDER_MODE_NONE], [.SLIDE_BORDER_MODE_TO_PARENT],
    ...
    private var manager: Handler? = null
    ...
    fun stopAutoScroll() {
        isAutoScroll = false
        manager!!.removeMessages(SCROLL_WHAT)
    }
    ...
    private fun sendScrollMessage(delayTimeInMills: Long) {
        ...
        manager!!.removeMessages(SCROLL_WHAT)
        manager!!.sendEmptyMessageDelayed(SCROLL_WHAT, delayTimeInMills)
    }
    ...
    fun scrollOnce() {
        ...
        if (adapter == null || totalCount == null || totalCount <= 1) {
            return
        }

        val nextItem = if (way == LEFT) --currentItem else ++currentItem
        if (nextItem < 0) {
            if (isCycle) {
    ...
    companion object {
        ...
        private class MyHandler(private val autoScrollViewPager: AutoScrollViewPager) : Handler() {
